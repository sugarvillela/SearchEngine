...
class urlFinder(object):
    ...
    def urlFix(self, url):
        ...
        if url[:4]=='http' and not ('www' in url):
            return url;
        if verbose and not ('www' in url):# too much output
            print( "fixing: ", url )
        errs=[ 'www.', '//', 'www,' ];
        for err in errs:
            index=url.find( err );
...
class urlLister(urlFinder):
    def __init__(self, types, skips, maxSize ):
        # set up list and types
        super(urlLister, self).__init__( types, skips, maxSize );
        self.badList=[];
        self.deadEnds=[];
    ...
    def dispFinal(self):
        if not verbose:
            return;
        print('final urls len =', len(self.urls) );
        print('dup ratio between pages =', self.dupRatio );
        print('overall dup ratio =', ( 1-len(self.urls)/self.maxSize) );
        print('badlist');
    ...
    def serialize( self, filename="urllist.txt" ):
        with open( invindex.fixPath( filename ), "w" ) as f:
            for u in self.urls:
    ...
    def deserialize( self, filename="urllist.txt" ):
        self.urls=[];
        with open( invindex.fixPath( filename ), "r" ) as f:
...
def testUrlLister():
    ...
    followTypes=['.edu'];
    skips=['.pdf', '.ico', '.png'];
    U=urlLister( followTypes, skips, 50 );
    U.parseAll( seedlist );
    #U.serialize();
    #U.disp();
